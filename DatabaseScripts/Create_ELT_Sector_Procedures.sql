-- =============================================================================
-- Election Tracker — [ELT].[Sector] stored procedures
--   * [ELT].[GetSectorList]  — list sectors, optionally filtered by BoothId.
--   * [ELT].[SectorSearch]   — paged/sorted dynamic search (see SectorRepository +
--                              CommonUtl.SetDynamicParametersData for the XML shape).
-- Target: each TENANT database. Engine: SQL Server.
-- =============================================================================

-- GetSectorList -------------------------------------------------------------
CREATE OR ALTER PROCEDURE [ELT].[GetSectorList]
    @BoothId INT = 0
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [ELT].[SectorSearch]
...
BEGIN
    ...
    IF (@SortColumn IS NULL OR LTRIM(RTRIM(@SortColumn)) = N'') SET @SortColumn = N'SectorId';
    IF (UPPER(ISNULL(@SortOrder, N'ASC')) NOT IN (N'ASC', N'DESC')) SET @SortOrder = N'ASC';

    SET @sql =
        N'SELECT *, COUNT(*) OVER() AS [TotalRecord] FROM [ELT].[Sector]' + @where +
