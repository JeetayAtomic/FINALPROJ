-- =============================================================================
-- Election Tracker — [ELT].[Ward] stored procedures
--   * [ELT].[GetWardList]  — list wards, optionally filtered by ConstituencyId.
--   * [ELT].[WardSearch]   — paged/sorted dynamic search (see WardRepository +
--                            CommonUtl.SetDynamicParametersData for the XML shape).
-- Target: each TENANT database. Engine: SQL Server.
-- =============================================================================

-- GetWardList ---------------------------------------------------------------
CREATE OR ALTER PROCEDURE [ELT].[GetWardList]
    @ConstituencyId INT = 0
AS
BEGIN
    SET NOCOUNT ON;
    SELECT *
    FROM [ELT].[Ward]
    WHERE (@ConstituencyId = 0 OR [ConstituencyId] = @ConstituencyId)
    ORDER BY [WardId];
END
GO
...
--   <xDynamicSearchParameters>
--     <_xDynamicSearchParameter DynamicColumnName="..." DynamicColumnValue="..." Operator="=" />
--   </xDynamicSearchParameters>
-- @PageNo is 1-based; 0 or 1 both return the first page.
CREATE OR ALTER PROCEDURE [ELT].[WardSearch]
...
BEGIN
    ...
    IF (@SortColumn IS NULL OR LTRIM(RTRIM(@SortColumn)) = N'') SET @SortColumn = N'WardId';
    IF (UPPER(ISNULL(@SortOrder, N'ASC')) NOT IN (N'ASC', N'DESC')) SET @SortOrder = N'ASC';

    SET @sql =
        N'SELECT *, COUNT(*) OVER() AS [TotalRecord] FROM [ELT].[Ward]' + @where +
